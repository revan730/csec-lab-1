...
const PolyalphabeticCipher = require('./cipher');
...
const calculateFitness = (genome, cipherText) => {
    let fitness = 0.0;
    // TODO: Replace with polyalphabetic cipher
    const cipher = new PolyalphabeticCipher();
    cipher.setAlphabet('ABCDEFGHIJKLMNOPQRSTUVWXYZ');
    cipher.setKey(genome);
...
class GeneticCracker {
    constructor(cipherText, keySize) {
    ...
    createInitialPopulation() {
        for (let i = 0;i < this.populationSize;i++) {
            // TODO: Replace with random string of alphachars of key length
            const genome = this.randomKey();
